...
// VecString represents a vector V[0...KeysNum), and each value V[i] represents a string
...
// at most TotalLen + KeysNum * (2 + log_2 TotalLen / KeysNum) / 8 bytes.
type VecString interface {
	...
	// KeysNum returns the number of strings
	Num() uint64
	...
	// PushBack set V[KeysNum] = v, and KeysNum++
	PushBack(v string)
}
